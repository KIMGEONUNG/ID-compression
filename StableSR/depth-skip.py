...
def main():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/stableSRNew/depthskip_text_T_512.yaml",
    ...
    model.register_schedule(given_betas=np.array(new_betas),
            ...
            with model.ema_scope():
                ...
                for n in trange(niters, desc="Sampling"):
                    ...
                    x_samples = vq_model.decode(
                        samples * 1. / model.scale_factor, enc_fea_lq)
                    if opt.colorfix_type == 'adain':
                        x_samples = adaptive_instance_normalization(
                            x_samples, init_image)
                    elif opt.colorfix_type == 'wavelet':
                        x_samples = wavelet_reconstruction(
                            x_samples, init_image)
                    ...
                    for i in range(init_image.size(0)):
                        ...
                        x_sample = 255. * rearrange(x_samples[i].cpu().numpy(),
                                                    'c h w -> h w c')
                        Image.fromarray(x_sample.astype(np.uint8)).save(
                                os.path.join(outpath, basename +  f'_d{opt.depth:02}.png'))
